...
class Problems extends React.Component {
  ...
  queryUntilDeleted(problemId, retries = 5, callback) {
    if (retries === 0) {
      console.log('failed');
      callback();
      return;
    ...
    this.getProblem(problemId)
      .then(response => {
        if (response.status === 200) {
          ...
          setTimeout(() => this.queryUntilDeleted(problemId, retries - 1, callback), 2000);
  ...
  onRemove(problemId, title) {
    this.openConfirmDialog({ description: `Do you really want to delete task: "${title}"` }, () => {
      this.setState((prevState) => {

        fetch(PROBLEMS_URL + "?challengeId=" + problemId, {
          method: 'DELETE',
          headers: { 'Content-Type': 'application/json', 'Authorization': this.context.jwtToken },
          mode: 'cors'
        })
          ...
          .then(response => {
            console.log('deleted');
            setTimeout(() => {
              this.queryUntilDeleted(problemId, 5, () => {
        ...
        <div>
          ...
          <TableContainer component={Paper}>
            <Table sx={{ minWidth: 650 }} size="small" aria-label="simple table">
              ...
              <TableBody>
                {this.filterItems(this.state.problems, this.state.filterTerm).sort((a, b) => a.title.localeCompare(b.title, undefined, { numeric: true })).map((problem, index) => (
                  <TableRow
                    key={index}
                  ...
                  >
                    ...
                    <TableCell align="left">
                      <RouteLink to={`/edit-problem/${problem.id}`} style={{ textDecoration: 'none' }}>
                        <IconButton disabled={this.state.loading} color="primary"><ModeEditIcon /></IconButton>
                      </RouteLink>
                      <RouteLink to={`/copy-problem/${problem.id}`} style={{ textDecoration: 'none' }}>
                        <IconButton disabled={this.state.loading} color="primary"><ContentCopyIcon /></IconButton>
                      </RouteLink>
                      <IconButton disabled={this.state.loading} color="primary" onClick={() => this.onRemove(problem.id, problem.title)}><DeleteIcon /></IconButton>
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
